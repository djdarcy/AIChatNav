...
Single source of truth: aichatnav/_version.py (MAJOR, MINOR, PATCH, PHASE).
...
Adapted from wtf-restarted's sync-versions.py for browser extension use.
...
VERSION_SOURCE = "aichatnav/_version.py"
MANIFEST_FILE = "src/manifest.json"
CHANGELOG_FILE = "CHANGELOG.md"
REPO_URL = "https://github.com/djdarcy/aichatnav"
TAG_PREFIX = "v"
# --------------------------------
...
def write_manifest_version(
    manifest_path: Path, version: str, dry_run: bool = False
) -> bool:
    """Update the version field in manifest.json (M.M.P format only)."""
    if not manifest_path.exists():
        return False
    content = manifest_path.read_text(encoding="utf-8")
    original = content
    # Replace the "version": "X.Y.Z" field
    content = re.sub(
        r'("version"\s*:\s*")[^"]+(")',
        f"\\g<1>{version}\\g<2>",
        content,
...
def main():
    parser = argparse.ArgumentParser(
        description="Sync versions across aichatnav project files"
    )
    parser.add_argument(
    ...
    if manifest_path.exists():
        manifest_ver = f"{components['major']}.{components['minor']}.{components['patch']}"
        current_manifest = read_manifest_version(manifest_path)
        if current_manifest != manifest_ver:
            all_synced = False
            if args.check:
                print(f"  [X] {MANIFEST_FILE}: version out of date ({current_manifest} != {manifest_ver})")
            else:
                updated = write_manifest_version(manifest_path, manifest_ver, args.dry_run)
                if updated:
                    action = "would update" if args.dry_run else "updated"
                    if not quiet:
                        print(f"  [OK] {MANIFEST_FILE}: version {action} to {manifest_ver}")
                    files_updated.append(MANIFEST_FILE)
        else:
            if args.verbose:
                print(f"  [OK] {MANIFEST_FILE}: version in sync ({manifest_ver})")
    else:
        if args.verbose:
            print(f"  [--] {MANIFEST_FILE}: not found (skipped)")
    ...
    if changelog_path.exists():
        ...
        if not args.check:
            ...
            # Match compare URL (...tag) for subsequent releases,
            # or releases/tag/ URL for the first release (no prior tag to compare)
            compare_pattern = rf"^\[{re.escape(human_ver)}\]:.*\.\.\.{re.escape(tag)}$"
            first_release_pattern = rf"^\[{re.escape(human_ver)}\]:.*releases/tag/{re.escape(tag)}$"
            if not (re.search(compare_pattern, content, re.MULTILINE) or
                    re.search(first_release_pattern, content, re.MULTILINE)):
                all_synced = False
